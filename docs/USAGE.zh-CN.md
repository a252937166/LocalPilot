...
从 0.7.6 起，`file://` 使用本机绝对路径。workspace 模式只允许配置的项目目录，拒绝保留目录、符号链接和硬链接；full_machine 仍允许访问其他目录，但检查 LocalPilot 自身的受保护路径。检测到当前主页面或内嵌 frame 是受限本机 URL 时，拒绝快照、截图和 JS 调用，并隐藏该标签的标题；截图和快照重试会复查跳转后的页面。可以关闭受限标签，或用导航工具跳回正常网页。这些是工具入口和结果检查，不是 Chrome 进程的操作系统沙箱；HTTP 网站及网页子资源仍由 Chrome 和本机网络权限处理。
